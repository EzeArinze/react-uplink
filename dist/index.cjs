...
function useOffline(options = {}) {
	const store = (0, react.useMemo)(() => getOrCreateStore(options), [configKeyStable(options)]);
	const subscribe = (0, react.useCallback)((onStoreChange) => store.subscribe(onStoreChange), [store]);
	const getSnapshot = (0, react.useCallback)(() => store.getSnapshot(), [store]);
	const getServerSnapshot = (0, react.useCallback)(() => SERVER_SNAPSHOT, []);
	const state = (0, react.useSyncExternalStore)(subscribe, getSnapshot, getServerSnapshot);
	const onStatusChangeRef = (0, react.useRef)(options.onStatusChange);
	onStatusChangeRef.current = options.onStatusChange;
	const prevOnlineRef = (0, react.useRef)(null);
	(0, react.useEffect)(() => {
		if (!isBrowser()) return;
